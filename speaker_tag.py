...
每一轮的 speaker 只能是四种之一:
- "operator"  画外的拍摄者在说 → 画面里所有人闭嘴
- 某个在册角色名 → 只有他对口型,其他人闭嘴
- "overlap"   两人及以上【同时】在说(08-23 新增)→ 驱动音轨里保留原音,
  宁可让人对一下口型也不能削 —— 削了会把其中一方的声音一起抹掉
...
"text":"这一轮的台词【逐字原文】(听不清的字用~代替;人审核对和时间码校准都靠它)","confidence":"高|中|低"}]}
...
def main():
    ...
    def resolve(nm):
        ...
        if str(nm).lower() == "operator":
            return "operator"
        if str(nm).lower() == "overlap":      # 两人同时说:合法取值,不归角色
            return "overlap"
        for r in roles:
            if nm == r["name"]:
                return r["name"]
