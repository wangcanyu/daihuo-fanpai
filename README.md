...
- 计费:即梦 CLI 走月度积分池(预付,用不完过期);火山 Ark 按 token;RunningHub 海螺 h3 按钱包实扣。**排产看池子状态而非单价**:月池有余额时烧即梦(现金支出为零,慢速档能让同一笔已付的钱多产 75% 素材),池子干了再走 h3(¥0.48/秒 + 真并发)。各档单价见「更新日志」。
